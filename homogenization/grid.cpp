...
void homo::Grid::readDensity(const std::string& fname, std::vector<float>& values, int reso[3], VoxelIOFormat frmat) {
	...
	if (frmat == homo::binary) {
		...
		// checkout resolution
		for (int i = 0; i < 3; i++) {
			reso[i] = 1 + *std::max_element(fpos[i].begin(), fpos[i].end());
		}
		// sort in lexicorder
...
void homo::Grid::v3_read(const std::string& filename, VT* v[3]) {
	...
	for (int i = 0; i < n_gsvertices(); i++) {
		...
		u[0].emplace_back(u_vert[0]);
		u[1].emplace_back(u_vert[1]);
		u[2].emplace_back(u_vert[2]);
	}
	ifs.close();
	for (int i = 0; i < 3; i++) {
...
Eigen::SparseMatrix<double> homo::Grid::stencil2matrix(bool removePeriodDof /*= true*/) {
	...
	if (!is_root) {
			...
			for (int j = 0; j < 9; j++) {
				...
				for (int k = 0; k < n_gsvertices(); k++) {
					...
					if (removePeriodDof) {
						...
						if (vpos[0] >= cellReso[0] + 1 || vpos[1] >= cellReso[1] + 1 || vpos[2] >= cellReso[2] + 1)
							continue;
					}
					...
					if (removePeriodDof) {
						for (int kk = 0; kk < 3; kk++) {
							vpos[kk] = (vpos[kk] + cellReso[kk]) % cellReso[kk];
						}
					} else {
						bool outBound = false;
						for (int kk = 0; kk < 3; kk++) {
							outBound = outBound || vpos[kk] < 0 || vpos[kk] > cellReso[kk];
						}
						if (outBound)
							continue;
					}
		...
		for (int i = 0; i < eflags.size(); i++) {
			if (eflags[i].is_fiction() || eflags[i].is_period_padding())
				continue;
...
void homo::Grid::vgsid2lexpos_h(int gsid, int pos[3]) {
	...
	if (color == -1)
		throw std::runtime_error("illegal gsid");
...
int homo::Grid::vlexpos2vlexid_h(int lexpos[3], bool removePeriodDof /* = false*/) {
	int vreso[3];
	for (int i = 0; i < 3; i++)
		lexpos[i] = (lexpos[i] + cellReso[i]) % cellReso[i];
	if (removePeriodDof) {
		for (int i = 0; i < 3; i++) {
			vreso[i] = cellReso[i];
		}
