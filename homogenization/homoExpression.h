...
struct el_var_t
	...
	__host_device_func el_var_t(el_tensor_t& el_, Scalar& cij, Scalar& gij)
		: el(el_), Base(cij, gij) {
		//printf("elvar ctor : el = %p, cij = %p, gij = %p\n", &el, &cij, &gij);
	}
	__host_device_func el_var_t(const el_var_t& el2)
		: el(el2.el), Base((Base&)el2) {
...
using ElasticMatrix = elastic_tensor_t<float, Exp>;
