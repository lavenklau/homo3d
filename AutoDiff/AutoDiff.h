...
template<typename Arg>
struct is_linearterm { static constexpr bool value = false; };
template<typename Arg>
struct is_linearexp { static constexpr bool value = false; };
...
template<typename Arg>
constexpr bool is_linearterm_v = is_linearterm<Arg>::value;
template<typename Arg>
constexpr bool is_linearexp_v = is_linearexp<Arg>::value;
template<typename Arg>
struct LinT {};
template<typename Exp, typename T>
struct LinT<LinearTerm<Exp, T>> { using type = T; };
template<typename T, typename... Exps>
struct LinT<linear_exp_t<T, Exps...>> { using type = T; };

template<typename Arg>
struct is_var { static constexpr bool value = false; };
template<typename Arg>
struct is_rvar { static constexpr bool value = false; };
template<typename... Args>
struct is_var<var_exp_t<Args...>> {
	static constexpr bool value = true;
};
template<typename... Args>
struct is_rvar<rvar_exp_t<Args...>> {
	static constexpr bool value = true;
};
template<typename Arg>
constexpr bool is_var_v = is_var<Arg>::value;
template<typename Arg>
constexpr bool is_rvar_v = is_rvar<Arg>::value;
template<typename Arg>
using is_var_i = std::enable_if_t<is_var<Arg>::value, int>;
template<typename Arg>
using is_rvar_i = std::enable_if_t<is_rvar<Arg>::value, int>;
...
template<typename... Args>
using tuple = homostd::tuple<Args...>;
...
template<typename T>
struct type_repeater<T, 0> { using type = decltype(tuple<>()); };
...
struct is_exp { static constexpr bool value = std::is_convertible_v<ExpT, exp_base_t>; };
template<typename ExpT>
constexpr bool is_exp_v = is_exp<ExpT>::value;

template<typename Exp, typename T>
using if_exp_t = std::enable_if_t<is_exp_v<Exp>, T>;
...
template<typename Scalar, typename Derived>
struct diff_func_t {
	__host_device_func Scalar operator()(Scalar v) const {
		return valueAt(v);
	}
...
struct eye_func_t : diff_func_t<Scalar, eye_func_t<Scalar>> {
	...
	__host_device_func Scalar diffAt(Scalar v) const {
		return 1;
	}
};
...
struct sgm_func_t : diff_func_t<Scalar, sgm_func_t<Scalar>> {
	__host_device_func Scalar valueAt(Scalar v) const {
		return Scalar(1) / (Scalar(1) + exp_(-v));
	}
	__host_device_func Scalar diffAt(Scalar v) const {
		Scalar t = exp_(-v);
		return t / (Scalar(1) + t);
...
template<typename Scalar>
struct inv_func_t<Scalar, eye_func_t<Scalar>> : eye_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, exp_func_t<Scalar>> : log_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, log_func_t<Scalar>> : exp_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, sin_func_t<Scalar>> : asin_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, asin_func_t<Scalar>> : sin_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, cos_func_t<Scalar>> : acos_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, acos_func_t<Scalar>> : cos_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, tan_func_t<Scalar>> : atan_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, atan_func_t<Scalar>> : tan_func_t<Scalar> {};
template<typename Scalar>
struct inv_func_t<Scalar, pow_func_t<Scalar>> : pow_func_t<Scalar> {
	inv_func_t(const pow_func_t<Scalar>& pf)
		: pow_func_t<Scalar>(Scalar(1) / pf.p){};
};

template<typename Scalar>
struct addop_func_t : binop_func_t<Scalar, addop_func_t<Scalar>> {
	constexpr static Scalar Identity = 0;
	__host_device_func Scalar combine(Scalar v0, Scalar v1) const {
		return v0 + v1;
	}
	template<int N>
...
struct exp_data_t {
	Scalar diff_;
	Scalar value_;
	__host_device_func exp_data_t(Scalar val, Scalar dif)
		: value_(val), diff_(dif) {}
	__host_device_func exp_data_t(void) = default;
	__host_device_func Scalar value(void) const {
		return value_;
	}
	__host_device_func Scalar diff() const {
		return diff_;
	}
	__host_device_func Scalar& rvalue(void) {
		return value_;
	}
	__host_device_func Scalar& rdiff(void) {
		return diff_;
	}
	__host_device_func const Scalar& rvalue(void) const {
		return value_;
	}
	__host_device_func const Scalar& rdiff(void) const {
		return diff_;
	}
};
...
struct exp_t : public exp_base_t, public exp_data_t<subExp_t, Scalar>, public exp_method_t<subExp_t, Scalar> {
	...
	//exp_t(void) :Data(0, 0) {}
	__host_device_func exp_t(Scalar v, Scalar dif)
		: Data(v, dif) {}
...
struct LinearTerm : public exp_t<LinearTerm<opExp, Scalar>, Scalar> {
	...
	opExp op;
	Scalar w;
	__host_device_func LinearTerm(const opExp& op_, Scalar w_ = Scalar{1})
		: op(op_), w(w_) {}
...
template<typename Scalar, typename... opExps_t>
struct linear_exp_t : public exp_t<linear_exp_t<Scalar, opExps_t...>, Scalar> {
	using Base = exp_t<linear_exp_t<Scalar, opExps_t...>, Scalar>;
	tuple<LinearTerm<opExps_t, Scalar>...> terms;

	__host_device_func linear_exp_t(const decltype(terms)& terms_)
		: terms(terms_) {}
...
struct binary_exp_t
	...
	opExp1_t op1;
	opExp2_t op2;
	__host_device_func binary_exp_t(const opExp1_t& op1_, const opExp2_t& op2_)
		: op1(op1_), op2(op2_) {}
};
...
struct mul_exp_t
	...
	__host_device_func mul_exp_t(const opExp1_t& op1_, const opExp2_t& op2_)
		: Base(op1_, op2_) {}
	__host_device_func Scalar eval_imp(void) {
		return Base::op1.eval() * Base::op2.eval();
	}
	__host_device_func void backward_imp(Scalar lastdiff) {
...
struct div_exp_t
	...
	__host_device_func div_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: Base(op1, op2) {}
	__host_device_func Scalar eval_imp(void) {
		return Base::op1.eval() / Base::op2.eval();
	}
...
struct unary_exp_t
	...
	opExp_t op;
	__host_device_func unary_exp_t(const opExp_t& op_)
		: op(op_) {}
};
...
struct exp_exp_t
	...
	__host_device_func exp_exp_t(const opExp_t& op)
		: unary_exp_t<exp_exp_t, opExp_t, Scalar>(op) {}
	__host_device_func Scalar eval_imp(void) {
		return exp_(Base::op.eval());
	}
	__host_device_func void backward_imp(Scalar lastdiff) {
...
struct ln_exp_t
	...
	ln_exp_t(const opExp_t& op)
		: unary_exp_t<ln_exp_t, opExp_t, Scalar>(op) {}
	__host_device_func Scalar eval_imp(void) {
...
struct scalar_exp_t
	...
	__host_device_func scalar_exp_t(Scalar s)
		: exp_t<scalar_exp_t<Scalar>, Scalar>(s, 0.) {}
	__host_device_func Scalar eval_imp(void) {
		return Base::value();
	}
	__host_device_func void backward_imp(Scalar lastdiff) {}
...
struct var_exp_t
	...
	Scalar& var;
	Scalar& vardiff;
	__host_device_func var_exp_t(Scalar& var_, Scalar& vardiff_)
		: var(var_), vardiff(vardiff_) {
		//printf("var ctor       [v] = %p  [d] = %p\n", &var, &vardiff);
	}
	__host_device_func var_exp_t(const var_exp_t& vari2)
		: var(vari2.var), vardiff(vari2.vardiff){
							  //printf("var copy ctor  [v] = %p  [d] = %p\n", &vari2.var, &vari2.vardiff);
						  };
	//__host_device_func var_exp_t(const var_exp_t& vari2) = default;
	__host_device_func Scalar value(void) {
		return var;
	}
	__host_device_func Scalar diff(void) {
		return vardiff;
	}
	__host_device_func Scalar& rvalue(void) {
		return var;
	...
	__host_device_func void update(Scalar var_) {
		var = var_;
		vardiff = 0;
	}
	__host_device_func Scalar eval(void) {
...
struct rvar_exp_t
	...
	__host_device_func rvar_exp_t(Scalar& var_, Scalar& vardiff_)
		: Base(var_, vardiff_) {}
	__host_device_func rvar_exp_t(const rvar_exp_t& r2)
		: Base((const Base&)r2){};
	__host_device_func Scalar eval_imp(void) {
		//Base::rdiff() = 0;
		//printf("vardiff = %e\n", Base::diff());
...
struct var_exp_t<void, Scalar>
	...
	__host_device_func var_exp_t(Scalar var_)
		: Base(var_, 0) {}
	__host_device_func var_exp_t(const var_exp_t& vari2) = default;
	__host_device_func void update(Scalar var_) {
		Base::rvalue() = var_;
