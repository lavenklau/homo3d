...
struct ManagedTempBlock {
  ...
  public:
	...
	template<typename T>
	ArrayHostProxy<T> proxy(void) {
		return ArrayHostProxy<T>(data<T>());
	}
...
struct devArray_t {
	...
	__host__ __device__ DType* data(void) {
		return _data;
	}
	__host__ __device__ const DType* data(void) const {
		return _data;
	}
...
template<int... Ns>
struct nArgs { static constexpr int value = nArgs<Ns...>::value + 1; };

template<int N1, int... Ns>
struct nArgs<N1, Ns...> { static constexpr int value = nArgs<Ns...>::value + 1; };

template<>
struct nArgs<> { static constexpr int value = 0; };

template<int N1, int... Ns>
struct MassProduct { static constexpr int value = N1 * MassProduct<Ns...>::value; };
template<int N1>
struct MassProduct<N1> { static constexpr int value = N1; };

template<int N, int... Ns>
struct FirstArg { static constexpr int value = N; };

template<int N1, int... Ns>
struct LastArg { static constexpr int value = LastArg<Ns...>::value; };

template<int N>
struct LastArg<N> { static constexpr int value = N; };
...
struct GraftArray<T, N1, Ns...> {
	int _ldd;
	T* _ptr;
	__host__ __device__ GraftArray(T* pdata, int ldd)
		: _ptr(pdata), _ldd(ldd) {}
...
struct GraftArray<T> {
	T* _ptr;
	__host__ __device__ GraftArray(T* pdata)
		: _ptr(pdata) {}
...
#define USE_SHARED_MEM_REDUCE 1
	...
	if (threadIdx.x >= BlockSize / 2)
		partialRes[threadIdx.x - BlockSize / 2] = x;
	__syncthreads();
	if (threadIdx.x < BlockSize / 2)
		partialRes[threadIdx.x] = red(partialRes[threadIdx.x], x);
	__syncthreads();
	if (BlockSize / 2 >= 128) {
...
#pragma unroll
	...
	if (BlockSize / 32 > 32)
		printf("\033[31m BlockSize should be less than 1024\033[0m\n");
	if (warpId == 0) {
		x = voidValue;
...
Tout norm(T* in_datax, T* in_datay, T* in_dataz, size_t n, Tout* sum_dst = nullptr) {
	...
	Tout nrm = sqrt(sum);
	if (sum_dst != nullptr) {
		cudaMemcpy(sum_dst, &nrm, sizeof(T), cudaMemcpyHostToDevice);
	}
	return nrm;
#else
...
T parallel_max(const T* indata, size_t array_size, T* max_dst = nullptr) {
	...
	cuda_error_check;
	if (max_dst != nullptr)
		cudaMemcpy(max_dst, pres, sizeof(T), cudaMemcpyDeviceToDevice);
	T res;
...
struct AbsWrapT {
	...
	__host__ __device__ AbsWrapT(T val_)
		: val(val_) {}
...
T parallel_min(const T* indata, size_t array_size, T* min_dst = nullptr) {
	...
	cudaDeviceSynchronize();
	if (min_dst != nullptr)
		cudaMemcpy(min_dst, pres, sizeof(T), cudaMemcpyDeviceToDevice);
	T res;
	cudaMemcpy(&res, pres, sizeof(T), cudaMemcpyDeviceToHost);
	cuda_error_check;
	return res;
...
T parallel_sum(const T* indata, size_t array_size, T* sum_dst = nullptr) {
	...
	if (sum_dst != nullptr)
		cudaMemcpy(sum_dst, &s, sizeof(T), cudaMemcpyHostToDevice);
	return s;
}
...
double parallel_sum_d(const T* indata, size_t array_size, T* sum_dst = nullptr) {
	double s = dump_map_sum(
		indata, [=] __device__(T in) { return in; }, array_size, double(0));
	if (sum_dst != nullptr)
		cudaMemcpy(sum_dst, &s, sizeof(T), cudaMemcpyHostToDevice);
	return s;
}
...
T parallel_diffdot(int v3size, T* v1[3], T* v2[3], T* v3[3], T* v4[3]) {
	...
	auto v3diffdot = [=] __device__(int tid) {
		return (_v2[0][tid] - _v1[0][tid]) * (_v4[0][tid] - _v3[0][tid]) + (_v2[1][tid] - _v1[1][tid]) * (_v4[1][tid] - _v3[1][tid]) + (_v2[2][tid] - _v1[2][tid]) * (_v4[2][tid] - _v3[2][tid]);
	};
...
struct array_t {
	T* _ptr;
	size_t _len;
	__host__ __device__ array_t(T* ptr, size_t len)
		: _ptr(ptr), _len(len) {}
...
namespace cub {
template<typename T>
struct NumericTraits<culib::AbsWrapT<T>> : NumericTraits<T> {
