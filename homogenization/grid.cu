...
// USE_DOUBLE_STENCIL
__constant__ double* rxstencil[27][9];
__constant__ double* rxCoarseStencil[27][9];
__constant__ double* rxFineStencil[27][9];
...
//#endif
	...
	initSharedMem(&sumKeU[0][0][0], sizeof(sumKeU) / sizeof(sumKeU[0][0][0]));
	initSharedMem(&sumKs[0][0][0], sizeof(sumKs) / sizeof(sumKs[0][0][0]));
	...
	if (!fiction && !vflag.is_period_padding()) {
		...
		for (int noff : {0, 14}) {
			...
			if (!neiflag.is_fiction()) {
				double u[3] = { gU[0][neighId],gU[1][neighId],gU[2][neighId] };
...
__global__ void restrict_stencil_kernel_1(
	...
	double pr = coarseRatio[0] * coarseRatio[1] * coarseRatio[2];
			...
			for (int xoff = -coarseRatio[0] + 1; xoff < coarseRatio[0]; xoff++) {
				for (int yoff = -coarseRatio[1] + 1; yoff < coarseRatio[1]; yoff++) {
					for (int zoff = -coarseRatio[2] + 1; zoff < coarseRatio[2]; zoff++) {
						...
						// USE_DOUBLE_STENCIL
						double wi = (coarseRatio[0] - abs(xoff))*(coarseRatio[1] - abs(yoff))*(coarseRatio[2] - abs(zoff)) / pr;
						int neighId = -1;
						if (nondyadic) {
						...
						if (neighId != -1) {
							...
							if (!vfineflag.is_fiction()) {
								for (int k = 0; k < 27; k++) {
									// USE_DOUBLE_STENCIL
									double rxf = rxFineStencil[k][i][neighId] * wi;
									int inCoarse[3] = {
									...
									for (int j = 0; j < 8; j++) {
										...
										double wj = (coarseRatio[0] - abs(wpos[0] - jpos[0])) *
...
void homo::Grid::restrict_stencil(void)
{
	...
	if (fine->assemb_otf) {
		...
		for (int i = 0; i < 27; i++) {
			for (int j = 0; j < 9; j++) {
				// USE_DOUBLE_STENCIL
				cudaMemset(stencil_g[i][j], 0, sizeof(double) * n_gsvertices());
			}
		}
		cudaDeviceSynchronize();
		cuda_error_check;
...
void homo::Grid::lexi2gsorder(double* src, double* dst, LexiType type_, bool lexipadded /*= false*/) {
	if (type_ == VERTEX) {
		devArray_t<int, 3> reso{ cellReso[0] + 1, cellReso[1] + 1, cellReso[2] + 1 };
		devArray_t<int, 8>  gsend;
		int nv = reso[0] * reso[1] * reso[2];
...
{
	// USE_DOUBLE_STENCIL
	auto tmpname = getMem().addBuffer(n_gsvertices() * sizeof(double));
	double* tmp = getMem().getBuffer(tmpname)->data<double>();
	for (int i = 0; i < 27; i++) {
		for (int j = 0; j < 9; j++) {
			cudaMemset(tmp, 0, sizeof(double) * n_gsvertices());
			cudaDeviceSynchronize();
			cuda_error_check;
			lexi2gsorder(stencil_g[i][j], tmp, VERTEX);
			cudaMemcpy(stencil_g[i][j], tmp, sizeof(double) * n_gsvertices(), cudaMemcpyDeviceToDevice);
		}
	}
...
__global__ void v3_stencilOnLeft_kernel(
	...
	__shared__ int gsCellEnd[8];
	__shared__ int gsVertexEnd[8];
#pragma push_macro("USE_LAME_MATRIX")
#undef USE_LAME_MATRIX
#if  USE_LAME_MATRIX
	__shared__ Lame KLAME[24][24];
#else
	// USE_DOUBLE_STENCIL
...
#pragma pop_macro("USE_LAME_MATRIX")
}
