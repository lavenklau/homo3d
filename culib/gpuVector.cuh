...
__global__ void block_sum_kernel(const Tin* pdata, Tout* odata, size_t n) {
	...
	if (tid < 32) {
		warpReduce<Tout, blockSize>(sdata, tid);
	}
	if (tid == 0)
...
template<typename, typename>
struct map_exp_t;
template<typename T = float, typename std::enable_if<std::is_scalar<T>::value, int>::type = 0>
struct scalar_t;
...
template<typename Scalar>
gVector<Scalar> gVector<Scalar>::buf_vector;
...
class gVectorMap
  ...
  public:
	gVectorMap(Scalar* data_ptr, size_t size)
		: gVector<Scalar>(data_ptr, size) {}
...
class gElementProxy {
  ...
  public:
	explicit gElementProxy(Scalar* ptr)
		: address(ptr) {}
...
void apply_vector(gVector<Scalar>& v1, const gVector<Scalar>& v2, Lambda func) {
	if (v2.size() != v1.size())
		printf("warning : using two vectors with unmatched size !");
...
struct exp_scalar { typedef typename exp::value_type type; };
template<typename exp>
using exp_scalar_t = typename exp_scalar<exp>::type;
template<typename _T1, typename _T2>
struct scalar_result { typedef decltype(_T1{0} * _T2{0}) type; };
template<typename _T1, typename _T2>
using scalar_result_t = typename scalar_result<_T1, _T2>::type;
...
struct var_t
	...
	const Scalar* ptr;
	int vec_dim;
	__host__ __device__ var_t(const Scalar* ptr_)
		: ptr(ptr_) {}
	__host__ __device__ var_t(const gVector<Scalar>& var)
		: ptr(var.data()), vec_dim(var.size()) {}
...
struct scalar_t
	...
	T scalar;
	__host__ __device__ scalar_t(T s)
		: scalar(s) {}
...
struct dup_exp_t
	...
	int nwordvalid;
	typedef ScalarType Scalar;
	__host__ __device__ dup_exp_t(const opExp_t& opexp, int nwvalid, int nwpitch, int ndp)
		: exp(opexp), nwordvalid(nwvalid), nwordpitch(nwpitch), ndup(ndp) {}
	__host__ __device__ int size(void) const {
		if (!transpose) {
...
struct slice_exp_t
	...
	opExp_t exp0;
	size_t start, end;
	__host__ __device__ slice_exp_t(const opExp_t& op, size_t start_id, size_t end_id)
		: exp0(op), start(start_id), end(end_id) {}
	__host__ __device__ int size(void) const {
		return end - start;
	}
	__device__ Scalar eval(int eid) const {
...
struct unary_exp_t
	...
	typedef ScalarType Scalar;
	opExp_t exp;
	__host__ __device__ unary_exp_t(const opExp_t& opexp)
		: exp(opexp) {}
	__host__ __device__ int size(void) const {
		return exp.size();
	}
...
struct negat_exp_t
	...
	__host__ __device__ negat_exp_t(const opExp_t& ex)
		: unary_exp_t<exp_scalar_t<opExp_t>, negat_exp_t<opExp_t>, opExp_t>(ex) {}
	__device__ Scalar eval(int eid) const {
...
struct abs_exp_t
	...
	typedef typename baseType::Scalar Scalar;
	__host__ __device__ abs_exp_t(const opExp_t& ex)
		: baseType(ex) {}
	__device__ Scalar eval(int eid) const {
...
struct sqrt_exp_t
	...
	typedef typename baseType::Scalar Scalar;
	__host__ __device__ sqrt_exp_t(const opExp_t& ex)
		: unary_exp_t<exp_scalar_t<opExp_t>, sqrt_exp_t<opExp_t>, opExp_t>(ex) {}
	__device__ Scalar eval(int eid) const {
...
struct binary_exp_t
	...
	opExp1_t exp1;
	opExp2_t exp2;
	__host__ __device__ binary_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: exp1(op1), exp2(op2) {}
	__host__ __device__ int size(void) const {
...
struct Less
	...
	typedef DT1 T1;
	typedef DT2 T2;
	__host__ __device__ bool compare(DT1 var1, DT2 var2) const {
		return var1 < var2;
	}
};
...
struct cmp_exp_t
	...
	opExp1_t exp1;
	opExp2_t exp2;
	__host__ __device__ cmp_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: exp1(op1), exp2(op2) {}
	__host__ __device__ int size(void) const {
		return (std::max)(exp1.size(), exp2.size());
	}
	__device__ bool eval(int eid) const {
		return CompareOp()(exp1.eval(eid), exp2.eval(eid));
	}
};
...
struct bool_exp_t
	...
	__host__ __device__ int size(void) const {
		return static_cast<const subExp*>(this)->size();
	}
	__device__ bool eval(int eid) const {
...
struct and_exp_t
	...
	opExp1_t exp1;
	opExp2_t exp2;
	__host__ __device__ and_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: exp1(op1), exp2(op2) {}
	__device__ bool eval(int eid) const {
		return exp1.eval(eid) && exp2.eval(eid);
	}
	__host__ __device__ int size(void) const {
		return (std::max)(exp1.size(), exp2.size());
	}
...
struct or_exp_t
	...
	opExp1_t exp1;
	opExp2_t exp2;
	__host__ __device__ or_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: exp1(op1), exp2(op2) {}
	__device__ bool eval(int eid) const {
		return exp1.eval(eid) || exp2.eval(eid);
	}
...
struct xor_exp_t
	...
	opExp1_t exp1;
	opExp2_t exp2;
	__host__ __device__ xor_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: exp1(op1), exp2(op2) {}
	__device__ bool eval(int eid) const {
		return exp1.eval(eid) ^ exp2.eval(eid);
	}
...
struct add_exp_t
	...
	__host__ __device__ add_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: binary_exp_t<add_exp_t, opExp1_t, opExp2_t>(op1, op2) {}
...
struct minus_exp_t
	...
	__host__ __device__ minus_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: binary_exp_t<minus_exp_t, opExp1_t, opExp2_t>(op1, op2) {}
...
struct div_exp_t
	...
	__host__ __device__ div_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: binary_exp_t<div_exp_t, opExp1_t, opExp2_t>(op1, op2) {}
...
struct multiply_exp_t
	...
	__host__ __device__ multiply_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: binary_exp_t<multiply_exp_t /*<opExp1_t, opExp2_t>*/, opExp1_t, opExp2_t>(op1, op2) {}
...
struct pow_exp_t
	...
	__host__ __device__ pow_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: binary_exp_t<pow_exp_t, opExp1_t, opExp2_t>(op1, op2) {}
...
struct min_exp_t
	...
	typedef typename baseType::Scalar Scalar;
	__host__ __device__ min_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: binary_exp_t<min_exp_t, opExp1_t, opExp2_t>(op1, op2) {}
	__device__ Scalar eval(int eid) const {
...
struct max_exp_t
	...
	typedef typename baseType::Scalar Scalar;
	__host__ __device__ max_exp_t(const opExp1_t& op1, const opExp2_t& op2)
		: binary_exp_t<max_exp_t, opExp1_t, opExp2_t>(op1, op2) {}
	__device__ Scalar eval(int eid) const {
		Scalar val1 = baseType::exp1.eval(eid);
		Scalar val2 = baseType::exp2.eval(eid);
		return val1 > val2 ? val1 : val2;
...
#define DEFINE_BINEXP(op, opsign)        \
	DEFINE_BINEXP_SCALAR_VAR(op, opsign) \
...
#define DEFINE_CMP_EXP(CompareName, op)    \
	DEFINE_CMP_SCALAR_VAR(CompareName, op) \
...
#define DEFINE_LOGIC_OP(opname, opsign)      \
	DEFINE_LOGIC_OP_EXP_EXP(opname, opsign)  \
