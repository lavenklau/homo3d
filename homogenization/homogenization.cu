...
#pragma unroll
	...
	__syncthreads();
	if (warpId < 4) {
		gSum[0][vlaneid] = (gSum[0][vlaneid] + c) * prho;
	}
	__syncthreads();
	if (warpId < 2) {
		gSum[0][vlaneid] += gSum[0][vlaneid + 64];
	}
	__syncthreads();
	// warp reduce
	if (warpId < 1) {
...
__global__ void fillHalfVertices_kernel(
	...
	for (int i = 0; i < N; i++) {
		...
		if (whichhalf == 1) {
			base[0] += alignedOffset;
			base[1] += alignedOffset;
			base[2] += alignedOffset;
		}
		for (int j = 0; j < 3; j++) {
...
__global__ void Sensitivity_kernel_opt_2(
	...
	if (!is_ghost)
		vflag = vflags[vid];
	if (vflag.is_fiction() || vflag.is_period_padding() || vflag.is_min_boundary())
		is_ghost = true;
