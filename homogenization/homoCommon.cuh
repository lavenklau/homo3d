...
struct Lame {
	short2 lammu;
	__host__ __device__ float lam() volatile {
		return lammu.x;
	}
	__host__ __device__ float mu() volatile {
		return lammu.y;
	}
...
struct GridVertexIndex {
	...
	struct IDColor {
		...
		__device__ int getId(void) {
			return cuda::std::get<0>(neigh_color);
		}
		__device__ int getColor(void) {
			return cuda::std::get<1>(neigh_color);
		}
	};
	...
	__device__ IDColor neighVertex(int id, volatile int* gsVertexEnd, volatile int gsVertexResoAll[3][8] /*, bool debug = false*/) {
		...
		if (neigh_setid >= 8 || neigh_setid < 0) {
			printf("error%d\n", __LINE__);
		}
